...
export function generateRandomSentence() {
    ...
    const randomNounIndex = Math.floor(Math.random() * dictionary.noun.length)
    const randomColorIndex = Math.floor(Math.random() * dictionary.noun.length)

    const adjective = dictionary.adjective[randomAdjectiveIndex];
    const noun = dictionary.noun[randomNounIndex];
    const color = colors[randomColorIndex].name;
